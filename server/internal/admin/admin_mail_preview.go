...
func (admin *API) mailPreview(writer http.ResponseWriter, request *http.Request) {
	if err := requireAdminMutation(request); err != nil {
		WriteAdminError(writer, 403, err.Error())
		return
	}
	var body struct {
		Mail    AdminMailRequest `json:"mail"`
		UserIDs []int            `json:"user_ids"`
	}
	if err := decodeAdminJSON(request, &body); err != nil {
	...
	if len([]rune(body.Mail.Title)) < 1 || len([]rune(body.Mail.Title)) > 40 || len([]rune(body.Mail.Message)) < 1 || len([]rune(body.Mail.Message)) > 200 || len(body.UserIDs) < 1 || len(body.UserIDs) > 10000 {
		WriteAdminError(writer, 400, "标题、正文或收件名单无效（最多 10000 个账号）")
		return
	}
	...
	for _, id := range body.UserIDs {
		if !known[id] {
			WriteAdminError(writer, 400, "收件名单包含不存在的账号")
			return
		}
		if !seen[id] {
			seen[id] = true
	...
	sort.Ints(users)
	WriteAdminJSON(writer, 200, map[string]any{"state": "PASS", "user_ids": users, "mail": body.Mail, "reward": reward, "catalog": entry, "total_quantity": int64(len(users)) * int64(body.Mail.Quantity)})
}
