...
function runtimeSettingsDirty(){return runtimeSettings.value!==null&&($('#settings-crystal').checked!==runtimeSettings.value.crystal_purchase_enabled||Number($('#settings-battle-speed').value)!==runtimeSettings.value.team_battle_speed||JSON.stringify(itemShopDraft())!==JSON.stringify(runtimeSettings.value.item_shop||[]))}
function renderRuntimeSettings(){
  ...
  $('#settings-version').textContent=`版本 ${runtimeSettings.revision}`;
...
$('#settings-battle-speed').onchange=()=>$('#settings-save').disabled=!runtimeSettingsDirty();
...
$('#settings-save').onclick=async()=>{
  ...
  try{
    const data=await api('/api/settings',{method:'PUT',body:JSON.stringify({crystal_purchase_enabled:$('#settings-crystal').checked,team_battle_speed:Number($('#settings-battle-speed').value),item_shop:itemShop,expected_revision:runtimeSettings.revision})});
