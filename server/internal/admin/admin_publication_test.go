...
func TestAdminPublicationRejectsStalePages(t *testing.T) {
	for _, kind := range []string{"boss", "gacha"} {
		t.Run(kind, func(t *testing.T) {
			accounts := testfixture.NewFriendCapacityTestAccounts(t)
			operations, err := NewOperations(accounts.Database(), []gamestate.GachaProfile{
			...
			read := func(response *httptest.ResponseRecorder) adminPublicationState {
				t.Helper()
				var result struct {
			...
			first, err := operations.storage.ReadDocument(key)
			if err != nil {
				t.Fatal(err)
			}
			secondID := 2 - winner
			...
			var firstIDs []int
			if kind == "boss" {
				state, decodeErr := adminPublicationFromDocument(first)
