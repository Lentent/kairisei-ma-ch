...
func TestMergeCNNormalQuestAreasPromotesCanonicalFirstArea(t *testing.T) {
	...
	if err := mergeCNNormalQuestAreas(&state, []json.RawMessage{canonical}); err != nil {
		t.Fatalf("merge normal quest areas: %v", err)
	}
	defaultAreaID, err := cnStageQuestAreaID(state.MainQuest)
...
func TestCollectCNNormalQuestClearedBossStates(t *testing.T) {
	...
	states, err := collectCNNormalQuestClearedBossStates([]json.RawMessage{area})
	if err != nil {
		t.Fatal(err)
	}
