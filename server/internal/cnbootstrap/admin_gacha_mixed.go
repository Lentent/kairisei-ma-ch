...
func (admin *cnAdmin) validateMixedGachaConfig(base release.GachaProfile, config cnAdminGachaConfig) (map[string]any, error) {
	if len(config.CardIDs) != 0 || len(config.Weights) != 0 || len(config.Steps) != len(base.Steps) {
		return nil, errors.New("混合池须保留预设奖励与阶段数量")
	}
	configured := cnAdminConfiguredGacha(base, config).Profile
	if err := release.ValidateGachaRules(configured); err != nil {
		return nil, err
	}
	canonical := [][]release.WeightedReward{base.RewardPool}
	edited := [][]release.WeightedReward{config.RewardPool}
	...
	for i, pool := range edited {
		...
		for j, entry := range pool {
			...
			key := cnAdminCatalogKey(entry.Reward.Type, entry.Reward.RewardTypeID)
			item, ok := admin.catalogByKey[key]
			if !ok {
	...
	stages := []httpapi.GachaOddsStage{}
	if len(config.Steps) == 0 {
		var err error
		stages, err = httpapi.PreviewGachaStages(configured, nil)
		if err != nil {
			return nil, err
		}
	} else {
		for i := range config.Steps {
			...
			rows, err := httpapi.PreviewGachaStages(configured, nil)
			if err != nil {
				return nil, err
			}
	...
	return map[string]any{"config": config, "base": base, "odds_scaled": []int{}, "odds_scale": 100000, "rarities": map[int]int{}, "blocked_card_ids": blocked, "publishable": len(blocked) == 0, "warnings": []string{"混合池奖励与赠礼按预设保留；各阶段价格和权重为本地运营配置。最后阶段重复，赠礼仍按累计抽取次数限制。"}, "stages": stages, "reward_names": names}, nil
}
