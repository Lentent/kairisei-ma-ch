...
const maxCNExploreMasterBytes = 1024 * 1024

type cnExploreMap struct {
	ID      int    `json:"id"`
...
type cnExploreRuntimeMaster struct {
	SchemaVersion      int                    `json:"schema_version"`
	ClientProfile      string                 `json:"client_profile"`
	Source             json.RawMessage        `json:"source"`
	Maps               []cnExploreMap         `json:"maps"`
	ExcludedTestMapIDs []int                  `json:"excluded_test_map_ids"`
	Stage              release.ExploreStage   `json:"stage"`
	Stages             []release.ExploreStage `json:"stages"`
	FloorRarity        int                    `json:"floor_rarity"`
	Events             []json.RawMessage      `json:"events"`
}

func loadCNExploreRuntimeMaster(masterPath string) (cnExploreRuntimeMaster, error) {
	if masterPath == "" {
		return cnExploreRuntimeMaster{}, errors.New("CN Explore runtime master path is required")
	}
	absolute, err := filepath.Abs(masterPath)
	if err != nil {
		return cnExploreRuntimeMaster{}, fmt.Errorf("resolve CN Explore runtime master: %w", err)
	}
	file, err := os.Open(absolute)
	if err != nil {
		return cnExploreRuntimeMaster{}, fmt.Errorf("open CN Explore runtime master: %w", err)
	}
	defer file.Close()
	info, err := file.Stat()
	if err != nil {
		return cnExploreRuntimeMaster{}, fmt.Errorf("stat CN Explore runtime master: %w", err)
	}
	if !info.Mode().IsRegular() || info.Size() <= 0 || info.Size() > maxCNExploreMasterBytes {
		return cnExploreRuntimeMaster{}, errors.New("CN Explore runtime master must be a non-empty regular JSON file within the size limit")
	}
	decoder := json.NewDecoder(io.LimitReader(file, maxCNExploreMasterBytes+1))
	decoder.DisallowUnknownFields()
	var master cnExploreRuntimeMaster
	if err := decoder.Decode(&master); err != nil {
		return cnExploreRuntimeMaster{}, fmt.Errorf("decode CN Explore runtime master: %w", err)
	}
	if err := requireJSONEOF(decoder); err != nil {
		return cnExploreRuntimeMaster{}, err
	}
	if err := validateCNExploreRuntimeMaster(master); err != nil {
		return cnExploreRuntimeMaster{}, err
	}
	return master, nil
}

func validateCNExploreRuntimeMaster(master cnExploreRuntimeMaster) error {
	...
	if len(stages) == 0 {
		stages = []release.ExploreStage{master.Stage}
	}
...
func applyCNExploreRuntimeMaster(state *release.State, master cnExploreRuntimeMaster) error {
	state.Explore.Stage = master.Stage
	state.Explore.Stage.Floors = append([]release.ExploreFloor(nil), master.Stage.Floors...)
	stages := master.Stages
	if len(stages) == 0 {
		stages = []release.ExploreStage{master.Stage}
	}
	state.Explore.Stages = make([]release.ExploreStage, len(stages))
	knownStageIDs := make(map[int]struct{}, len(stages))
	for index, stage := range stages {
		state.Explore.Stages[index] = stage
		state.Explore.Stages[index].Floors = append([]release.ExploreFloor(nil), stage.Floors...)
	...
	state.Explore.Avatar = release.ExploreAvatar{
