...
const maxCNNaviMasterBytes = 1024 * 1024

type cnNaviDefinition struct {
...
type cnNaviLocalPurchase struct {
	Evidence string `json:"evidence"`
	Currency string `json:"currency"`
	Price    int    `json:"price"`
}

type cnNaviPublicationPolicy struct {
	Evidence             string `json:"evidence"`
...
type cnNaviRuntimeMaster struct {
	SchemaVersion int                     `json:"schema_version"`
	ClientProfile string                  `json:"client_profile"`
	Source        json.RawMessage         `json:"source"`
	Publication   cnNaviPublicationPolicy `json:"publication_policy"`
	LocalPurchase cnNaviLocalPurchase     `json:"local_purchase"`
	Summary       cnNaviSummary           `json:"summary"`
	Navigators    []cnNaviDefinition      `json:"navigators"`
}

func loadCNNaviRuntimeMaster(masterPath string) (cnNaviRuntimeMaster, error) {
	if masterPath == "" {
		return cnNaviRuntimeMaster{}, errors.New("CN navigator runtime master path is required")
	}
	absolute, err := filepath.Abs(masterPath)
	if err != nil {
		return cnNaviRuntimeMaster{}, fmt.Errorf("resolve CN navigator runtime master: %w", err)
	}
	file, err := os.Open(absolute)
	if err != nil {
		return cnNaviRuntimeMaster{}, fmt.Errorf("open CN navigator runtime master: %w", err)
	}
	defer file.Close()
	info, err := file.Stat()
	if err != nil {
		return cnNaviRuntimeMaster{}, fmt.Errorf("stat CN navigator runtime master: %w", err)
	}
	if !info.Mode().IsRegular() || info.Size() <= 0 || info.Size() > maxCNNaviMasterBytes {
		return cnNaviRuntimeMaster{}, errors.New("CN navigator runtime master must be a non-empty regular JSON file within the size limit")
	}
	decoder := json.NewDecoder(io.LimitReader(file, maxCNNaviMasterBytes+1))
	decoder.DisallowUnknownFields()
	var master cnNaviRuntimeMaster
	if err := decoder.Decode(&master); err != nil {
		return cnNaviRuntimeMaster{}, fmt.Errorf("decode CN navigator runtime master: %w", err)
	}
	if err := requireJSONEOF(decoder); err != nil {
		return cnNaviRuntimeMaster{}, err
	}
	if err := validateCNNaviRuntimeMaster(master); err != nil {
		return cnNaviRuntimeMaster{}, err
	}
	return master, nil
}

func validateCNNaviRuntimeMaster(master cnNaviRuntimeMaster) error {
...
func applyCNNaviRuntimeMaster(state *release.State, master cnNaviRuntimeMaster) error {
