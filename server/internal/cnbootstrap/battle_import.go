...
type cnBattleImportedEntry struct {
...
func cnBattleImportedEntries(master cnBattleRuntimeMaster) (map[int]cnBattleImportedEntry, error) {
	var profile struct {
		Import struct {
			Entries []cnBattleImportedEntry `json:"entries"`
		} `json:"jp_boss_import"`
	}
	...
	entries := make(map[int]cnBattleImportedEntry, len(profile.Import.Entries))
