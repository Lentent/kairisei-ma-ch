...
async function loadView(name,force=false){if(policyBusy(name)||(state.loaded.has(name)&&!force))return;if(name==='pool-editor'&&force&&poolEditor.dirty&&!confirm('重新载入会放弃未保存的卡池编辑，是否继续？'))return;if(force&&typeof adminPolicyDirty==='function'&&adminPolicyDirty(name)&&!confirm('当前发布设置尚未保存，刷新会放弃修改。是否继续？'))return;state.loading.add(name);updatePolicyControls();try{if(name==='player-policy'){await loadPlayerPolicy()}else if(name==='dashboard'){const status=await api('/api/status');state.status=status;renderDashboard()}else if(name==='accounts'){await loadAccountsPage()}else if(name==='mail'){await loadMailWorkspace()}else if(name==='bosses'){await loadBossPublication(state.bossCatalog)}else if(name==='gachas'){const [presets,policy]=await Promise.all([api('/api/gacha-presets'),api('/api/gacha-policy')]);state.gachaPresets=presets.presets;state.gachaPolicy=policy.publication;state.gachaSelected=new Set(state.gachaPolicy.group_ids||[]);renderGachas()}else if(name==='pool-editor'){await loadPoolEditor()}else if(name==='audit'){await loadAuditPage()}else if(name==='settings'){await loadRuntimeSettings()}else if(name==='drops'){await loadDropEditor()}else if(name==='exchanges'){await loadExchangeEditor()}state.loaded.add(name);if(force)toast('当前页面已刷新')}catch(e){toast(e.message,true)}finally{state.loading.delete(name);updatePolicyControls()}}
...
async function loadBossPublication(catalog){
  ...
  $('#boss-start').value=localTimeInput(state.policy.start_unix);$('#boss-end').value=localTimeInput(state.policy.end_unix);state.bossPage=0;renderBosses();
}
...
function filteredGroups(){const q=$('#boss-search').value.trim().toLowerCase();return state.groups.filter(g=>(state.bossKind==='all'||g.category===state.bossKind)&&(!q||`${g.name} ${g.past_name||''} ${g.group_id} ${g.picture_id} ${g.boss_ids.join(' ')} ${(g.difficulties||[]).join(' ')}`.toLowerCase().includes(q)))}
