...
async function loadView(name,force=false){if(policyBusy(name)||(state.loaded.has(name)&&!force))return;if(name==='pool-editor'&&force&&poolEditor.dirty&&!confirm('重新载入会放弃未保存的卡池编辑，是否继续？'))return;if(force&&typeof adminPolicyDirty==='function'&&adminPolicyDirty(name)&&!confirm('当前发布设置尚未保存，刷新会放弃修改。是否继续？'))return;state.loading.add(name);updatePolicyControls();try{if(name==='player-policy'){await loadPlayerPolicy()}else if(name==='dashboard'){const status=await api('/api/status');state.status=status;renderDashboard()}else if(name==='accounts'){await loadAccountsPage()}else if(name==='mail'){await loadMailWorkspace()}else if(name==='bosses'){const [groups,policy]=await Promise.all([api('/api/boss-groups'),api('/api/boss-policy')]);state.groups=groups.groups;state.policy=policy.publication;state.mode=state.policy.mode;state.selected=new Set(state.policy.group_ids||[]);$('#boss-start').value=localTimeInput(state.policy.start_unix);$('#boss-end').value=localTimeInput(state.policy.end_unix);renderBosses()}else if(name==='gachas'){const [presets,policy]=await Promise.all([api('/api/gacha-presets'),api('/api/gacha-policy')]);state.gachaPresets=presets.presets;state.gachaPolicy=policy.publication;state.gachaSelected=new Set(state.gachaPolicy.group_ids||[]);renderGachas()}else if(name==='pool-editor'){await loadPoolEditor()}else if(name==='audit'){await loadAuditPage()}else if(name==='settings'){await loadRuntimeSettings()}else if(name==='drops'){await loadDropEditor()}else if(name==='exchanges'){await loadExchangeEditor()}state.loaded.add(name);if(force)toast('当前页面已刷新')}catch(e){toast(e.message,true)}finally{state.loading.delete(name);updatePolicyControls()}}
function renderDashboard(){const s=state.status,p=s.publication;$('#m-state').textContent=s.state;$('#m-endpoint').textContent=s.game_endpoint;$('#m-accounts').textContent=num(s.account_count);$('#m-bosses').textContent=num(s.boss_count);$('#m-groups').textContent=`${num(s.boss_group_count)} 个目录组`;$('#m-rooms').textContent=num(s.active_rooms);$('#deployment-facts').innerHTML=[['客户端 Profile',s.client_profile],['游戏 API',s.game_endpoint],['BattleSv 端口',s.battle_port],['SQLite schema',s.sqlite_schema_version],['服务端 UTC',s.server_time_utc]].map(([a,b])=>`<div class="fact"><span>${esc(a)}</span><code>${esc(b)}</code></div>`).join('');const now=Date.parse(s.server_time_utc)/1000;
  ...
  $('#policy-copy').textContent=`${scheduled?'等待排期开始':expired?'排期已结束，当前不发布活动 BOSS':'当前发布 '+selection}。普通副本与圣剑学习入口不受此设置影响。`;
}
...
function filteredGroups(){const q=$('#boss-search').value.trim().toLowerCase();return state.groups.filter(g=>(state.bossKind==='all'||g.category===state.bossKind)&&(!q||`${g.name} ${g.group_id} ${g.picture_id} ${g.boss_ids.join(' ')} ${(g.difficulties||[]).join(' ')}`.toLowerCase().includes(q)))}
...
function updatePolicyControls(){
  ...
  $('#save-policy').disabled=policyBusy('bosses')||!state.policy||(state.mode==='allowlist'&&!state.selected.size);
...
$('#save-policy').onclick=async()=>{
  ...
  if(state.mode==='allowlist'&&!state.selected.size)return toast('至少选择一个 Boss 组',true);
  ...
  if(!confirm(`发布预览：${body.mode==='all'?'全部开放':`仅开放 ${body.group_ids.length} 组`}\n开始：${describeTime(body.start_unix)}\n结束：${describeTime(body.end_unix)}\n确认发布目录排期？`))return;
  state.publishing.add('bosses');updatePolicyControls();
  try{
    const data=await api('/api/boss-policy',{method:'PUT',body:JSON.stringify(body)});
