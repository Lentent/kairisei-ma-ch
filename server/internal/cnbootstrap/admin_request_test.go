...
func TestAdminJSONRejectsBodyBeyondLimit(t *testing.T) {
	...
	for _, body := range []string{prefix + " ", prefix + `{}`} {
		...
		var payload struct{}
		if err := decodeCNAdminJSON(request, &payload); err == nil {
...
func TestAdminRejectsReboundHostBeforeServingAccounts(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.host, func(t *testing.T) {
			reached := false
			handler := cnAdminSecurityHeaders(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
				reached = true
...
func TestAdminMailIdempotencyComparesStoredContent(t *testing.T) {
	mail := cnAdminMailRequest{Title: "cards", Message: "reward"}
	reward := release.Reward{Type: 6, Num: 1, RewardTypeID: 1001, CardLevel: 1, CardFame: 1, CardSkillLevels: []int16{1}}
	existing := release.Present{Title: mail.Title, Comment: mail.Message, Reward: reward}
	if !cnAdminMailPayloadMatches(existing, mail, reward) {
		t.Fatal("identical retry rejected")
	}
	changed := reward
	changed.Num = 100
	if cnAdminMailPayloadMatches(existing, mail, changed) {
