...
const cnAccountRowsSchema = `
CREATE TABLE IF NOT EXISTS cn_account_balance (
...
CREATE TABLE IF NOT EXISTS cn_account_card (
...
CREATE TABLE IF NOT EXISTS cn_account_item (
 user_id INTEGER NOT NULL REFERENCES cn_account_balance(user_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS cn_account_stack_card (
 user_id INTEGER NOT NULL REFERENCES cn_account_balance(user_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS cn_account_present (
 user_id INTEGER NOT NULL REFERENCES cn_account_balance(user_id) ON DELETE CASCADE,
...
func syncCNAccountRows(tx *sql.Tx, table, key, columns string, userID int, incoming []cnStoredRow) error {
	...
	order := cnStableRowOrder(incoming, previousOrder)
...
func cnStableRowOrder(rows []cnStoredRow, previous map[int64]int64) []int64 {
...
const cnCardRowColumns = "slot,card_id,level,experience,now_exp,next_exp,love,fame,is_lock,hp,attack,magic,mind,base_add_price,skill_levels"
const cnItemRowColumns = "num,limit_time"
const cnStackRowColumns = "num,hp,attack,magic,mind,add_exp,base_add_price,material_type"
const cnPresentRowColumns = "history,issued_at_unix,add_elapsed_sec,limit_time,state,reason,title,comment,url,auto_fusion_used,auto_loveup_used,admin_key,rewards_json"

func writeCNAccountRows(tx *sql.Tx, state release.State) error {
	u := state.User
 ...
 OR friend_point<>excluded.friend_point OR pvp_point<>excluded.pvp_point`,
	...
	cards := make([]cnStoredRow, 0, len(state.Cards)+len(state.ContainerCards))
	for slot, inventory := range [][]release.Card{state.Cards, state.ContainerCards} {
		for _, c := range inventory {
			skills, err := json.Marshal(c.SkillLevels)
			if err != nil {
				return err
			}
			cards = append(cards, cnStoredRow{c.UniqueID, []any{slot, c.CardID, c.Level, c.Experience, c.NowLevelExperience, c.NextLevelExperience, c.Love, c.Fame, c.IsLock, c.HP, c.Attack, c.Magic, c.Mind, c.BaseAddPrice, string(skills)}})
		}
	}
	if err := syncCNAccountRows(tx, "cn_account_card", "unique_id", cnCardRowColumns, u.UserID, cards); err != nil {
		return err
	}
	items := make([]cnStoredRow, 0, len(state.Items))
	for _, item := range state.Items {
		items = append(items, cnStoredRow{int64(item.ItemID), []any{item.Num, item.LimitTime}})
	}
	if err := syncCNAccountRows(tx, "cn_account_item", "item_id", cnItemRowColumns, u.UserID, items); err != nil {
		return err
	}
	stacks := make([]cnStoredRow, 0, len(state.StackCards))
	for _, c := range state.StackCards {
		stacks = append(stacks, cnStoredRow{int64(c.CardID), []any{c.Num, c.HP, c.Attack, c.Magic, c.Mind, c.AddExperience, c.BaseAddPrice, c.MaterialType}})
	}
	if err := syncCNAccountRows(tx, "cn_account_stack_card", "card_id", cnStackRowColumns, u.UserID, stacks); err != nil {
		return err
	}
	presents := make([]cnStoredRow, 0, len(state.Engagement.Presents)+len(state.Engagement.Histories))
	for history, group := range [][]release.Present{state.Engagement.Presents, state.Engagement.Histories} {
		for _, p := range group {
			// Only the four small, typed reward descriptors are variable JSON.
			rewards, err := json.Marshal([4]release.Reward{p.Reward, p.Reward0, p.Reward1, p.Reward2})
			if err != nil {
				return err
			}
			presents = append(presents, cnStoredRow{p.PresentID, []any{history, p.IssuedAtUnix, p.AddElapsedSec, p.LimitTime, p.State, p.Reason, p.Title, p.Comment, p.URL, p.AutoFusionUsed, p.AutoLoveupUsed, p.AdminIdempotencyKey, string(rewards)}})
		}
	}
	return syncCNAccountRows(tx, "cn_account_present", "present_id", cnPresentRowColumns, u.UserID, presents)
}
...
func readCNAccountRows(tx *sql.Tx, state *release.State) error {
	u := &state.User
	...
	templates := make(map[int]release.Card, len(state.CardTemplates)+len(state.Cards))
	for _, inventory := range [][]release.Card{state.Cards, state.ContainerCards, state.CardTemplates} {
		for _, card := range inventory {
			templates[card.CardID] = card
		}
	}
	state.Cards, state.ContainerCards = []release.Card{}, []release.Card{}
	...
	if err := read("cn_account_card", "unique_id", cnCardRowColumns, func(rows *sql.Rows) error {
		var c release.Card
		var slot int
		var skills string
	...
	if err := read("cn_account_item", "item_id", cnItemRowColumns, func(rows *sql.Rows) error {
	...
	if err := read("cn_account_stack_card", "card_id", cnStackRowColumns, func(rows *sql.Rows) error {
		var c release.CardStack
	...
	state.Engagement.Presents, state.Engagement.Histories = []release.Present{}, []release.Present{}
	return read("cn_account_present", "present_id", cnPresentRowColumns, func(rows *sql.Rows) error {
		var p release.Present
		var history int
		var raw string
