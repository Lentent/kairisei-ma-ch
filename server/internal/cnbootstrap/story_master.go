...
const maxCNStoryMasterBytes = 4 * 1024 * 1024

type cnStoryMasterStory struct {
...
type cnStoryMasterSection struct {
	StoryMainSectionID int                  `json:"story_main_sectionid"`
	SectionTitle       string               `json:"section_title"`
	Stories            []cnStoryMasterStory `json:"stories"`
}

type cnStoryMasterPart struct {
	StoryMainPartID int                    `json:"story_main_partid"`
	Name            string                 `json:"name"`
	Sections        []cnStoryMasterSection `json:"sections"`
}

type cnStoryMasterSubStory struct {
	StorySubID         int            `json:"story_subid"`
	StateFlag          int            `json:"state_flag"`
	Title              string         `json:"title"`
	UnlockText         string         `json:"unlock_text"`
	FeatureReward      release.Reward `json:"feature_reward"`
	ScriptSegmentCount int            `json:"script_segment_count"`
}

type cnStoryMasterSubSection struct {
	StorySubSectionID   int                     `json:"story_sub_sectionid"`
	StorySubSectionType int                     `json:"story_sub_section_type"`
	SectionTitle        string                  `json:"section_title"`
	PictID              int                     `json:"pictid"`
	Stories             []cnStoryMasterSubStory `json:"stories"`
}

type cnStoryMasterSubCharacter struct {
	StoryMainCharacterID int                       `json:"story_main_charaid"`
	Name                 string                    `json:"name"`
	PictID               int                       `json:"pictid"`
	Sections             []cnStoryMasterSubSection `json:"sections"`
}

type cnStoryMasterEventStory struct {
	Sub       cnStoryMasterSubStory         `json:"sub"`
	Materials []release.StoryUnlockMaterial `json:"materials"`
}

type cnStoryMasterEvent struct {
	StoryEventID int                       `json:"story_eventid"`
	Name         string                    `json:"name"`
	PictID       int                       `json:"pictid"`
	Stories      []cnStoryMasterEventStory `json:"stories"`
}

type cnStoryEventMapping struct {
	StoryEventID        int      `json:"story_eventid"`
...
type cnStorySubCharacterContainerContract struct {
	Evidence                     string                    `json:"evidence"`
	...
	PublishedContainer           cnStoryMasterSubCharacter `json:"published_container"`
}

type cnStoryRuntimeMaster struct {
	SchemaVersion                 int                                  `json:"schema_version"`
	ClientProfile                 string                               `json:"client_profile"`
	Source                        json.RawMessage                      `json:"source"`
	NormalParts                   []cnStoryMasterPart                  `json:"normal_parts"`
	CNParts                       []cnStoryMasterPart                  `json:"cn_parts"`
	SubCharacters                 []cnStoryMasterSubCharacter          `json:"sub_characters"`
	SubCharacterContainerContract cnStorySubCharacterContainerContract `json:"sub_character_container_contract"`
	SubSectionMappings            []cnStorySubSectionMapping           `json:"sub_section_mappings"`
	Events                        []cnStoryMasterEvent                 `json:"events"`
	EventMappings                 []cnStoryEventMapping                `json:"event_mappings"`
	RewardPolicy                  release.StoryRewardPolicy            `json:"reward_policy"`
	EventPageProfile              release.EventPageProfile             `json:"event_page_profile"`
	PopupProfile                  release.PopupProfile                 `json:"popup_profile"`
	StoryBattles                  cnStoryBattleCatalog                 `json:"story_battles"`
...
func loadCNStoryRuntimeMaster(masterPath string) (cnStoryRuntimeMaster, error) {
	if masterPath == "" {
		return cnStoryRuntimeMaster{}, errors.New("CN story runtime master path is required")
	}
	absolute, err := filepath.Abs(masterPath)
	if err != nil {
		return cnStoryRuntimeMaster{}, fmt.Errorf("resolve CN story runtime master: %w", err)
	}
	file, err := os.Open(absolute)
	if err != nil {
		return cnStoryRuntimeMaster{}, fmt.Errorf("open CN story runtime master: %w", err)
	}
	defer file.Close()
	info, err := file.Stat()
	if err != nil {
		return cnStoryRuntimeMaster{}, fmt.Errorf("stat CN story runtime master: %w", err)
	}
	if !info.Mode().IsRegular() || info.Size() <= 0 || info.Size() > maxCNStoryMasterBytes {
		return cnStoryRuntimeMaster{}, errors.New("CN story runtime master must be a non-empty regular JSON file within the size limit")
	}
	decoder := json.NewDecoder(io.LimitReader(file, maxCNStoryMasterBytes+1))
	decoder.DisallowUnknownFields()
	var master cnStoryRuntimeMaster
	if err := decoder.Decode(&master); err != nil {
		return cnStoryRuntimeMaster{}, fmt.Errorf("decode CN story runtime master: %w", err)
	}
	if err := requireJSONEOF(decoder); err != nil {
		return cnStoryRuntimeMaster{}, err
	}
	if err := validateCNStoryRuntimeMaster(master); err != nil {
		return cnStoryRuntimeMaster{}, err
	}
	return master, nil
}

func validateCNStoryRuntimeMaster(master cnStoryRuntimeMaster) error {
	...
	normalCount, err := validateCNStoryMasterParts(master.NormalParts, make(map[int]struct{}))
	if err != nil {
		return fmt.Errorf("validate normal story catalog: %w", err)
	}
	cnCount, err := validateCNStoryMasterParts(master.CNParts, make(map[int]struct{}))
	if err != nil {
		return fmt.Errorf("validate CN story catalog: %w", err)
	}
	subCount, err := validateCNStoryMasterSubCharacters(
	...
	eventCount, err := validateCNStoryMasterEvents(master.Events, master.EventMappings)
	if err != nil {
		return fmt.Errorf("validate event story catalog: %w", err)
	}
	if err := validateCNStoryBattleCatalog(master); err != nil {
		return fmt.Errorf("validate story fixed-battle catalog: %w", err)
	}
	if err := validateCNStoryRewardPolicy(master); err != nil {
		return fmt.Errorf("validate story first-clear policy: %w", err)
	}
	profile := master.EventPageProfile
	if err := validateCNEventPageProfile(profile); err != nil {
		return err
	}
...
func validateCNStoryRewardPolicy(master cnStoryRuntimeMaster) error {
	...
	for name, reward := range map[string]release.Reward{
...
func validateCNStoryMasterEvents(
	events []cnStoryMasterEvent,
	mappings []cnStoryEventMapping,
) (int, error) {
...
func validateCNStoryBattleCatalog(master cnStoryRuntimeMaster) error {
...
func validateCNStoryMasterParts(parts []cnStoryMasterPart, storyIDs map[int]struct{}) (int, error) {
	if len(parts) == 0 {
...
func validateCNStoryMasterSubCharacters(
	characters []cnStoryMasterSubCharacter,
	mappings []cnStorySubSectionMapping,
	contract cnStorySubCharacterContainerContract,
) (int, error) {
	...
	mappingBySectionID := make(map[int]cnStorySubSectionMapping, len(mappings))
...
func materializeCNStoryParts(source []cnStoryMasterPart, persisted []release.StoryMainPart) []release.StoryMainPart {
	...
	parts := make([]release.StoryMainPart, len(source))
	for partIndex, part := range source {
		parts[partIndex] = release.StoryMainPart{
			StoryMainPartID: part.StoryMainPartID,
			Name:            part.Name,
			Sections:        make([]release.StoryMainSection, len(part.Sections)),
		}
		for sectionIndex, section := range part.Sections {
			parts[partIndex].Sections[sectionIndex] = release.StoryMainSection{
				StoryMainSectionID: section.StoryMainSectionID,
				SectionTitle:       section.SectionTitle,
				Stories:            make([]release.StoryMain, len(section.Stories)),
			}
			for storyIndex, story := range section.Stories {
				parts[partIndex].Sections[sectionIndex].Stories[storyIndex] = release.StoryMain{
...
func normalizeCNStoryProgress(parts []release.StoryMainPart) {
	firstUncleared := true
...
func materializeCNSubStories(
	source []cnStoryMasterSubCharacter,
	persisted []release.StorySubCharacter,
	cards []release.Card,
) []release.StorySubCharacter {
	progress := make(map[int]int)
	...
	for characterIndex, character := range source {
		...
		for sectionIndex, section := range character.Sections {
			...
			for storyIndex, story := range section.Stories {
				...
				characters[characterIndex].Sections[sectionIndex].Stories[storyIndex] = release.StorySub{
...
func materializeCNEventStories(
	source []cnStoryMasterEvent,
	persisted []release.StoryEvent,
) []release.StoryEvent {
	progress := make(map[int]int)
	...
	events := make([]release.StoryEvent, len(source))
	for eventIndex, event := range source {
		events[eventIndex] = release.StoryEvent{
			StoryEventID: event.StoryEventID,
			Name:         event.Name,
			PictID:       event.PictID,
			Stories:      make([]release.StorySubEvent, len(event.Stories)),
		}
		firstUncleared := true
		for storyIndex, wrapper := range event.Stories {
			story := wrapper.Sub
			...
			featureReward := story.FeatureReward
			featureReward.CardSkillLevels = append([]int16{}, story.FeatureReward.CardSkillLevels...)
			events[eventIndex].Stories[storyIndex] = release.StorySubEvent{
				Sub: release.StorySub{
					StorySubID:    story.StorySubID,
					StateFlag:     stateFlag,
					Title:         story.Title,
					UnlockText:    unlockText,
				...
				Materials: append([]release.StoryUnlockMaterial{}, wrapper.Materials...),
			}
		}
	}
	return events
...
func applyCNStoryRuntimeMaster(state *release.State, master cnStoryRuntimeMaster) bool {
	normalParts := materializeCNStoryParts(master.NormalParts, state.Story.MainParts)
	cnParts := materializeCNStoryParts(master.CNParts, state.Story.CNMainParts)
	ownedCards := make([]release.Card, 0, len(state.Cards)+len(state.ContainerCards))
	ownedCards = append(ownedCards, state.Cards...)
	ownedCards = append(ownedCards, state.ContainerCards...)
	subCharacters := materializeCNSubStories(
		master.SubCharacters,
	...
	release.UnlockCollectedCharacterStories(subCharacters, collected)
	events := materializeCNEventStories(master.Events, state.Story.Events)
	...
	state.EventPageProfile.Buttons = append([]release.EventPageButton{}, master.EventPageProfile.Buttons...)
	...
	state.Story.StoryBattleReferences = make([]release.StoryBattleReference, len(master.StoryBattles.References))
	for index, reference := range master.StoryBattles.References {
		state.Story.StoryBattleReferences[index] = release.StoryBattleReference{
