...
const paymentName = profile => ({2:'友情点',3:'水晶（先免费后付费）',4:`物品 ${profile.pay_typeid}`,6:'付费水晶'}[profile.pay_type] || `类型 ${profile.pay_type}`);
...
async function loadPoolEditor() {
  ...
  $('#pool-select').innerHTML = data.pools.map(row=>`<option value="${row.gacha_id}">${esc(row.config.name)} · ${row.gacha_id}</option>`).join('');
  if (data.pools.some(row=>row.gacha_id===previous)) $('#pool-select').value=previous;
  selectPool();
}

function selectPool() {
  poolEditor.editSerial++;
  poolEditor.row = poolEditor.rows.find(row=>row.gacha_id===Number($('#pool-select').value));
  const row=poolEditor.row;
  if (!row) {poolEditor.preview=null;updatePoolControls();return;}
  const config = row.draft.payload || row.config;
  $('#pool-enabled').checked=!config.disabled; $('#pool-name').value=config.name; $('#pool-price').value=config.price;
...
function poolMatches(card) {
  ...
  return (!poolEditor.job || card.arthur_type===poolEditor.job) && (!poolEditor.source || (card.source_tags||[]).includes(poolEditor.source)) && (!q || `${card.name} ${card.reward_type_id} ${card.pict_id} ${card.detail||''}`.toLowerCase().includes(q)) && (!rarity || card.rarity===rarity) && (!$('#pool-ready').checked || card.resource_state!=='unavailable');
}
function poolCardEligible(card) { const base=poolEditor.row?.base; if(base?.unowned_only && card.rarity!==6) return false; return base?.pay_type===2 || card.gacha_eligible || (!card.detail && card.rarity===6 && base?.pool_source_state==='INFERRED_OFFICIAL_CN_CACHED_BANNER_FEATURED_IDENTITIES_LOCAL_RARITY5_REMAINDER_CHR10_CHR20_CLOSED' && base.cardids.includes(card.reward_type_id)); }
...
function poolConfig() {
  ...
  return {...extra,disabled:!$('#pool-enabled').checked,gacha_id:poolEditor.row.gacha_id,name:$('#pool-name').value.trim(),price:poolEditor.mixed?.steps?.length?poolEditor.mixed.steps[0].price:Number($('#pool-price').value),start_unix:unixInput('#pool-start'),end_unix:unixInput('#pool-end'),card_ids:entries.map(e=>e[0]),weights:entries.map(e=>e[1])};
}
function renderPoolPreview(preview) {
  ...
  $('#pool-preview').innerHTML=`<h3>${esc(c.name)}</h3><p>${esc(paymentName(b))} × ${c.price} · ${b.card_num} 抽${b.daily_first_free?' · 每日首次免费':''}</p><p>开始：${describeTime(c.start_unix)}<br>结束：${describeTime(c.end_unix)}</p><p>${c.disabled?'此抽取方式已停用。':'此抽取方式已启用；还需整组开放且在排期内。'}</p><p>${preview.warnings.map(esc).join('<br>')}</p>${ready?'':'<p>未闭包卡牌：'+preview.blocked_card_ids.join(', ')+'</p>'}${distributions.map(stage=>`<details><summary>${esc(stage.name||'')} · ${stage.draw_count} 抽${stage.rarity?` · 稀有度 ${stage.rarity}`:''} · ${(stage.card_ids||stage.rewards||[]).length} 项 · 展开概率</summary><div class="table-wrap" style="max-height:280px"><table><thead><tr><th>卡牌</th><th>单次概率</th></tr></thead><tbody>${(stage.rewards||stage.card_ids||[]).map((r,i)=>`<tr><td>${esc(typeof r==='number'?(names.get(r)||r):rewardLabel(r))}${typeof r==='number'?'':` × ${r.num}`}</td><td>${(stage.odds_scaled[i]/preview.odds_scale).toFixed(5)}%</td></tr>`).join('')}</tbody></table></div></details>`).join('')}<p class="sub">这些是本地运营配置概率。</p>`;
  updatePoolControls();
}
$('#pool-select').onchange=()=>{if(policyBusy('pool-editor')){$('#pool-select').value=poolEditor.row?.gacha_id||'';return}if(poolEditor.dirty&&!confirm('切换卡池将放弃未保存的编辑，是否继续？')){$('#pool-select').value=poolEditor.row.gacha_id;return}selectPool()};
['#pool-enabled','#pool-name','#pool-price','#pool-start','#pool-end'].forEach(id=>$(id).oninput=poolChanged);
...
$('#pool-import').onchange=event=>runPoolEdit(async()=>{
  try {
    ...
    $('#pool-enabled').checked=!c.disabled;$('#pool-name').value=c.name;$('#pool-price').value=c.price;$('#pool-start').value=start;$('#pool-end').value=end;
    poolEditor.selected=selected;setupMixedPool(c);poolChanged();renderPoolCards();renderSelectedCards();
