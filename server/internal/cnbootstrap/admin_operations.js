...
function poolMatches(card) {
  ...
  return cardMatchesJob(card,poolEditor.job) && (!poolEditor.source || (card.source_tags||[]).includes(poolEditor.source)) && (!q || `${card.name} ${card.reward_type_id} ${card.pict_id} ${card.detail||''}`.toLowerCase().includes(q)) && (!rarity || card.rarity===rarity) && (!$('#pool-ready').checked || card.resource_state!=='unavailable');
}
