...
const maxCNMultiplayerCompletionBytes = 512 * 1024

func (accounts *cnAccountStore) NextRoomID(minimum int64) (int64, error) {
	if minimum <= 0 {
		return 0, errors.New("minimum multiplayer room ID is invalid")
	}
	accounts.mu.Lock()
	defer accounts.mu.Unlock()
	database, transaction, err := accounts.beginMultiplayerTransaction()
	if err != nil {
		return 0, err
	}
...
func (accounts *cnAccountStore) SaveCompleted(completed multiplayer.CompletedBattle, expiresAt time.Time) error {
	...
	if len(content) == 0 || len(content) > maxCNMultiplayerCompletionBytes {
...
func (accounts *cnAccountStore) LoadCompleted(roomID int64, now time.Time) (multiplayer.CompletedBattle, time.Time, error) {
...
func (accounts *cnAccountStore) beginMultiplayerTransaction() (*sql.DB, *sql.Tx, error) {
	...
	if err != nil {
		_ = database.Close()
		return nil, nil, fmt.Errorf("begin multiplayer SQLite transaction: %w", err)
	}
...
func (accounts *cnAccountStore) pruneMultiplayerCompletions(transaction *sql.Tx, now time.Time) error {
	if _, err := transaction.ExecContext(
		context.Background(),
...
func validateCompletedBattle(completed multiplayer.CompletedBattle, expiresAt time.Time) error {
	...
	for _, drop := range completed.ReleasedDrops {
		if completed.DropLedgerVersion != 1 || drop.BattleIndex < 0 || drop.BattleIndex > completed.BattleIndex || drop.EnemyIndex < 0 || drop.EnemyIndex >= 4 || drop.ChancePerMillion != nil || !validCNPersistedRewardShape(drop.Reward) {
