...
func (a *cnAdmin) bossCatalog(r *http.Request) (string, []cnAdminBattleGroup, map[int]struct{}, error) {
	switch r.URL.Query().Get("catalog") {
	case "", "activity":
		return cnTeamBattlePublicationKey, a.groups, a.knownGroups, nil
	case "past":
		known := make(map[int]struct{}, len(a.pastGroups))
		for _, g := range a.pastGroups {
			known[g.GroupID] = struct{}{}
		}
		return cnPastBattlePublicationKey, a.pastGroups, known, nil
	default:
		return "", nil, nil, errors.New("未知副本目录")
	}
}

func buildCNAdminPastGroups(rawGroups []json.RawMessage, groups []cnAdminBattleGroup, cards map[string]cnAdminCatalogEntry) ([]cnAdminBattleGroup, error) {
	byBoss := make(map[int]cnAdminBattleGroup)
	for _, g := range groups {
		for _, id := range g.BossIDs {
			byBoss[id] = g
		}
	}
	result := make([]cnAdminBattleGroup, 0, len(rawGroups))
	for _, raw := range rawGroups {
		...
		g := cnAdminBattleGroup{GroupID: past.ID, Name: past.Name, PastName: past.PastName, Category: "2d", BossIDs: []int{}, Difficulties: []string{}, SegmentCounts: []int{}}
		...
		if len(past.Cards) > 0 {
			card := cards[cnAdminCatalogKey(6, past.Cards[0])]
