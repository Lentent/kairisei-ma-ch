...
func (admin *cnAdmin) listAccountMail(writer http.ResponseWriter, request *http.Request) {
	userID, err := strconv.Atoi(chi.URLParam(request, "userID"))
	if err != nil || userID < cnPrimaryUserID || userID >= cnSystemPartnerUserIDBase {
		writeCNAdminError(writer, http.StatusBadRequest, "invalid player ID")
		return
	}
	...
	for i := len(state.Engagement.Presents) - 1; i >= 0; i-- {
		...
		entry := admin.catalogByKey[cnAdminCatalogKey(p.Reward.Type, p.Reward.RewardTypeID)]
	...
	writeCNAdminJSON(writer, http.StatusOK, map[string]any{"items": items, "total": total, "user_id": userID})
}

func (admin *cnAdmin) deleteAccountMail(writer http.ResponseWriter, request *http.Request) {
	...
	if err := decodeCNAdminJSON(request, &body); err != nil || len(body.PresentIDs) == 0 || len(body.PresentIDs) > 100 {
		writeCNAdminError(writer, http.StatusBadRequest, "请选择 1–100 封邮件")
		return
	}
	...
	for _, value := range body.PresentIDs {
		...
		if err != nil || id <= 0 || strconv.FormatInt(id, 10) != value || selected[id] {
			writeCNAdminError(writer, http.StatusBadRequest, "邮件 ID 无效或重复")
			return
		}
		selected[id] = true
	}
	...
	remaining := make([]release.Present, 0, len(state.Engagement.Presents))
	deleted := []string{}
	...
	for _, p := range state.Engagement.Presents {
		...
		p.State = release.PresentStateAdminDeleted
