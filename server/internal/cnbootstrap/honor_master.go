...
const maxCNHonorMasterBytes = 4 * 1024 * 1024

type cnHonorDefinition struct {
...
type cnHonorRuntimeMaster struct {
	SchemaVersion int                 `json:"schema_version"`
	ClientProfile string              `json:"client_profile"`
	Source        json.RawMessage     `json:"source"`
	Honors        []cnHonorDefinition `json:"honors"`
}

func loadCNHonorRuntimeMaster(masterPath string) (cnHonorRuntimeMaster, error) {
	if masterPath == "" {
		return cnHonorRuntimeMaster{}, errors.New("CN honor runtime master path is required")
	}
	absolute, err := filepath.Abs(masterPath)
	if err != nil {
		return cnHonorRuntimeMaster{}, fmt.Errorf("resolve CN honor runtime master: %w", err)
	}
	file, err := os.Open(absolute)
	if err != nil {
		return cnHonorRuntimeMaster{}, fmt.Errorf("open CN honor runtime master: %w", err)
	}
	defer file.Close()
	info, err := file.Stat()
	if err != nil {
		return cnHonorRuntimeMaster{}, fmt.Errorf("stat CN honor runtime master: %w", err)
	}
	if !info.Mode().IsRegular() || info.Size() <= 0 || info.Size() > maxCNHonorMasterBytes {
		return cnHonorRuntimeMaster{}, errors.New("CN honor runtime master must be a non-empty regular JSON file within the size limit")
	}
	decoder := json.NewDecoder(io.LimitReader(file, maxCNHonorMasterBytes+1))
	decoder.DisallowUnknownFields()
	var master cnHonorRuntimeMaster
	if err := decoder.Decode(&master); err != nil {
		return cnHonorRuntimeMaster{}, fmt.Errorf("decode CN honor runtime master: %w", err)
	}
	if err := requireJSONEOF(decoder); err != nil {
		return cnHonorRuntimeMaster{}, err
	}
	if err := validateCNHonorRuntimeMaster(master); err != nil {
		return cnHonorRuntimeMaster{}, err
	}
	return master, nil
}

func validateCNHonorRuntimeMaster(master cnHonorRuntimeMaster) error {
