...
func TestCNAdminAccountMutationAndAuditCommitTogether(t *testing.T) {
	...
	admin := &cnAdmin{accounts: accounts, business: newCNAccountBusinessRouter(nil, nil), progression: policy,
		catalogByKey: map[string]cnAdminCatalogEntry{"4:0": {Kind: "gold", Name: "金币"}}}
	...
	defer db.Close()
	for _, userID := range []int{cnPrimaryUserID, other.UserID} {
