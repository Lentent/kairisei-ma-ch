...
const cnOwnDeckBossIDOffset = 100000000

type cnBattleOwnDeckVariant struct {
	SourceBossID int `json:"source_boss_id"`
	BossID       int `json:"boss_id"`
}

func validateCNBattleOwnDeckVariants(master cnBattleRuntimeMaster) error {
	if len(master.OwnDeckVariants) == 0 {
		return nil
	}
	sources := make(map[int]cnBattleEntryRules)
	...
	for _, raw := range master.Groups {
		...
		for _, boss := range group.Bosses {
			sources[boss.BossID] = boss.cnBattleEntryRules
		}
	}
	...
	for id := range cnBattleCanonicalBossAliases(master.DeduplicatedFamilies) {
		reserved[id] = true
	}
	seen := make(map[int]bool)
	for _, variant := range master.OwnDeckVariants {
		rules, exists := sources[variant.SourceBossID]
		if !exists || variant.SourceBossID <= 0 || variant.SourceBossID >= cnOwnDeckBossIDOffset ||
			variant.BossID != variant.SourceBossID+cnOwnDeckBossIDOffset || reserved[variant.BossID] ||
...
func projectCNBattleOwnDeckVariants(master cnBattleRuntimeMaster) (cnBattleRuntimeMaster, error) {
	if err := validateCNBattleOwnDeckVariants(master); err != nil {
