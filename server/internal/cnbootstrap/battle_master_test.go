...
func TestCollectCNBattlePersistedStatesMigratesExactDuplicateFamily(t *testing.T) {
	...
	states, err := collectCNBattlePersistedStates(
...
func TestRetiredSeedBattleGroupsAreExplicit(t *testing.T) {
	for _, groupID := range []int{200000, 200001} {
		if !isCNBattleRetiredSeedGroupID(groupID) {
