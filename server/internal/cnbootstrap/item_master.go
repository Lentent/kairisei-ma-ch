...
const maxCNItemMasterBytes = 2 * 1024 * 1024

type cnItemRuntimeMaster struct {
	SchemaVersion             int                           `json:"schema_version"`
	ClientProfile             string                        `json:"client_profile"`
	Source                    json.RawMessage               `json:"source"`
	LocalAccountConfigVersion int                           `json:"local_account_config_version,omitempty"`
	LocalAccountInitialItems  []release.Item                `json:"local_account_initial_items,omitempty"`
	TeamBattleMedalItemID     int                           `json:"team_battle_medal_item_id,omitempty"`
	UserBuffProfiles          []release.UserBuffProfile     `json:"user_buff_profiles"`
	Items                     []release.ItemDefinition      `json:"items"`
	ItemGachaProfiles         []release.ItemGachaProfile    `json:"item_gacha_profiles"`
	ItemGachaSourceGapIDs     []int                         `json:"item_gacha_source_gap_ids"`
	ItemExchangeProfiles      []release.ItemExchangeProfile `json:"item_exchange_profiles"`
	ItemExchangeSourceStatus  string                        `json:"item_exchange_source_status"`
	ItemExchangePolicy        cnItemExchangePolicy          `json:"item_exchange_policy"`
	ItemLackTipProfiles       []release.ItemLackTipProfile  `json:"item_lack_tip_profiles"`
	ItemLackTipSourceStatus   string                        `json:"item_lack_tip_source_status"`
	ItemLackTipPolicy         cnItemLackTipPolicy           `json:"item_lack_tip_policy"`
	EventShopProfiles         []release.EventShopProfile    `json:"event_shop_profiles"`
	EventShopSourceStatus     string                        `json:"event_shop_source_status"`
	EventShopSourceAudit      cnEventShopSourceAudit        `json:"event_shop_source_audit"`
	TradeShopProfiles         []release.TradeShopProfile    `json:"trade_shop_profiles"`
	LocalTradeShopProfiles    []release.TradeShopProfile    `json:"local_trade_shop_profiles,omitempty"`
	TradeShopPolicy           cnTradeShopPolicy             `json:"trade_shop_policy"`
	TradeShopSourceStatus     string                        `json:"trade_shop_source_status"`
	SkippedInactiveItemIDs    []int                         `json:"skipped_inactive_item_ids"`
}

type cnTradeShopPolicy struct {
...
type cnItemExchangePolicy struct {
	ConfigVersion                int               `json:"config_version"`
...
type cnItemLackTipPolicy struct {
	ConfigVersion              int               `json:"config_version"`
...
type cnEventShopSourceAudit struct {
...
func loadCNItemRuntimeMaster(masterPath string) (cnItemRuntimeMaster, error) {
	if masterPath == "" {
		return cnItemRuntimeMaster{}, errors.New("CN item runtime master path is required")
	}
	absolute, err := filepath.Abs(masterPath)
	if err != nil {
		return cnItemRuntimeMaster{}, fmt.Errorf("resolve CN item runtime master: %w", err)
	}
	file, err := os.Open(absolute)
	if err != nil {
		return cnItemRuntimeMaster{}, fmt.Errorf("open CN item runtime master: %w", err)
	}
	defer file.Close()
	info, err := file.Stat()
	if err != nil {
		return cnItemRuntimeMaster{}, fmt.Errorf("stat CN item runtime master: %w", err)
	}
	if !info.Mode().IsRegular() || info.Size() <= 0 || info.Size() > maxCNItemMasterBytes {
		return cnItemRuntimeMaster{}, errors.New("CN item runtime master must be a non-empty regular JSON file within the size limit")
	}
	decoder := json.NewDecoder(io.LimitReader(file, maxCNItemMasterBytes+1))
	decoder.DisallowUnknownFields()
	var master cnItemRuntimeMaster
	if err := decoder.Decode(&master); err != nil {
		return cnItemRuntimeMaster{}, fmt.Errorf("decode CN item runtime master: %w", err)
	}
	if err := requireJSONEOF(decoder); err != nil {
		return cnItemRuntimeMaster{}, err
	}
	if err := validateCNItemRuntimeMaster(master); err != nil {
		return cnItemRuntimeMaster{}, err
	}
	return master, nil
}

func validateCNItemRuntimeMaster(master cnItemRuntimeMaster) error {
	...
	var source struct {
		UserBuff struct {
			...
			ContainerSHA256 string `json:"container_sha256"`
			TextAssetBytes  int64  `json:"textasset_bytes"`
			TextAssetSHA256 string `json:"textasset_sha256"`
		} `json:"user_buff"`
	}
	if json.Unmarshal(master.Source, &source) != nil ||
		source.UserBuff.ContainerPath != "assets/resources/container/user/user_buff.csv.txt" ||
		source.UserBuff.ContainerBytes <= 0 || len(source.UserBuff.ContainerSHA256) != 64 ||
		source.UserBuff.TextAssetBytes <= 0 || len(source.UserBuff.TextAssetSHA256) != 64 {
		return errors.New("CN item runtime master user-buff source is incomplete")
	}
	seen := make(map[int]struct{}, len(master.Items))
	definitions := make(map[int]release.ItemDefinition, len(master.Items))
	...
	for _, profile := range master.ItemGachaProfiles {
		...
		for _, reward := range profile.Rewards {
			if release.ValidateGachaReward(reward) != nil {
	...
	for _, profile := range master.ItemExchangeProfiles {
		...
		if _, exists := definitions[profile.ItemID]; !exists ||
			...
			!validCNItemExchangeReward(profile.Reward, definitions) {
	...
	if master.EventShopSourceStatus != "ABSENT_FROM_CN_OFFICIAL_CONTAINER_AND_PACKAGED_EVENT_SHOP_WEB_FILES" ||
		...
		len(master.EventShopSourceAudit.ContainerSHA256) != 64 ||
...
func applyCNItemRuntimeMaster(state *release.State, master cnItemRuntimeMaster) (bool, error) {
	state.ItemDefinitions = append([]release.ItemDefinition(nil), master.Items...)
	state.UserBuffProfiles = append([]release.UserBuffProfile(nil), master.UserBuffProfiles...)
	state.ItemGachaProfiles = append([]release.ItemGachaProfile(nil), master.ItemGachaProfiles...)
	state.ItemExchangeProfiles = append([]release.ItemExchangeProfile(nil), master.ItemExchangeProfiles...)
	state.ItemLackTipProfiles = cloneCNItemLackTipProfiles(master.ItemLackTipProfiles)
	state.EventShopProfiles = append([]release.EventShopProfile(nil), master.EventShopProfiles...)
	state.TradeShopProfiles = append([]release.TradeShopProfile(nil), master.TradeShopProfiles...)
...
func cloneCNItemLackTipProfiles(source []release.ItemLackTipProfile) []release.ItemLackTipProfile {
	result := make([]release.ItemLackTipProfile, len(source))
	for index, profile := range source {
		profile.TextURLs = make([]release.ItemLackTipLink, len(profile.TextURLs))
...
func validCNItemExchangeReward(reward release.Reward, definitions map[int]release.ItemDefinition) bool {
