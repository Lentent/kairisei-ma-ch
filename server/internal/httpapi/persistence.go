...
type StatePersister func(release.State) error

func (a *API) persist() error {
	state := a.store.snapshot(a.release.State)
...
func (s *store) snapshot(base release.State) release.State {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.snapshotLocked(base)
}

func (s *store) snapshotLocked(base release.State) release.State {
	...
	state.User.Jobs = append([]release.JobParameter(nil), s.currentJobs...)
	...
	state.Spheres = append([]release.Sphere{}, s.spheres...)
	state.Buddies = append([]release.Buddy{}, s.buddies...)
	state.User.BP = s.bp
	...
	state.Items = make([]release.Item, 0, len(s.items))
	...
	state.GachaSelections = make([]release.GachaSelection, 0, len(s.gachaSelections))
	for gachaID, rewards := range s.gachaSelections {
		state.GachaSelections = append(state.GachaSelections, release.GachaSelection{
			GachaID: gachaID,
	...
	state.GachaDailyClaims = make([]release.GachaDailyClaim, 0, len(s.gachaDailyClaims))
	for gachaID, day := range s.gachaDailyClaims {
		state.GachaDailyClaims = append(state.GachaDailyClaims, release.GachaDailyClaim{
			GachaID: gachaID,
			Day:     day,
		})
	}
	...
	state.EventShopPurchases = make([]release.EventShopPurchase, 0, len(s.eventShopPurchases))
	for lineupID, count := range s.eventShopPurchases {
		state.EventShopPurchases = append(state.EventShopPurchases, release.EventShopPurchase{
			LineupID: lineupID,
			Count:    count,
		})
	}
	...
	state.TradeShopPurchases = make([]release.TradeShopPurchase, 0, len(s.tradeShopPurchases))
	for lineupID, count := range s.tradeShopPurchases {
		state.TradeShopPurchases = append(state.TradeShopPurchases, release.TradeShopPurchase{
			LineupID: lineupID,
			Count:    count,
		})
	}
	...
	baseCards := make(map[int64]release.Card, len(base.Cards)+len(base.ContainerCards))
	for _, inventory := range [][]release.Card{base.Cards, base.ContainerCards} {
		for _, card := range inventory {
			baseCards[card.UniqueID] = card
		}
	}
	state.Cards = releaseCardsFromStore(s.cards, baseCards, s.cardDefinitions)
	state.ContainerCards = releaseCardsFromStore(s.containerCards, baseCards, s.cardDefinitions)
	state.StackCards = append([]release.CardStack(nil), s.stackCards...)
	state.CardDevelopment = release.CardDevelopmentState{Stive: s.stive}
	...
	state.Decks = make([]release.Deck, len(s.decks))
	for index, deck := range s.decks {
		state.Decks[index] = release.Deck{
	...
	state.ActiveTeamBattle = releaseTeamBattleContext(s.activeBattle)
	state.TeamBattleStartReceipts = append([]release.TeamBattleStartReceipt(nil), s.teamBattleStartReceipts...)
	state.TeamBattleContinueReceipts = append([]release.TeamBattleContinueReceipt(nil), s.teamBattleContinueReceipts...)
	...
	state.TeamBattleResultReceipts = make([]release.TeamBattleResultReceipt, 0, len(s.teamBattleReceipts))
	...
	state.TeamBattleSoloResultReceipts = make(
		[]release.TeamBattleSoloResultReceipt,
		0,
	...
	state.PVPResultReceipts = make([]release.PVPResultReceipt, 0, len(s.pvpResultReceipts))
	...
	state.TowerQuestProgress = make([]release.TowerQuestProgress, 0, len(s.towerQuestProgress))
...
func releaseCardsFromStore(
	cards []cardInfo,
	baseCards map[int64]release.Card,
