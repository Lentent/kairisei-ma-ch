...
func TestMultiplayerStartDebitIsDurableAndAtomic(t *testing.T) {
	s := &store{bp: 20, bpMax: 20, bpRecoveryInterval: 3 * time.Minute}
	...
	if err := s.chargeMultiplayerStart(start, release.State{}, persist); err != nil {
		t.Fatal(err)
	}
	if s.bp != 5 || saved.User.BP != 5 || len(saved.TeamBattleStartReceipts) != 1 {
		t.Fatal("start debit was not persisted with its receipt")
	}
	if err := s.chargeMultiplayerStart(start, release.State{}, persist); err != nil || writes != 1 || s.bp != 5 {
		t.Fatal("repeated start charged twice")
	}
	// Rehydrated debit state must also acknowledge the same accepted start.
	restarted := &store{bp: saved.User.BP, bpMax: 20, bpRecoveryInterval: 3 * time.Minute, teamBattleStartReceipts: append([]release.TeamBattleStartReceipt(nil), saved.TeamBattleStartReceipts...)}
	if err := restarted.chargeMultiplayerStart(start, release.State{}, persist); err != nil || writes != 1 || restarted.bp != 5 {
		t.Fatal("restart replay charged twice")
	}
	start.RoomID++
	if err := s.chargeMultiplayerStart(start, release.State{}, persist); err == nil || s.bp != 5 || writes != 1 {
		t.Fatal("insufficient balance start was committed")
	}
	start.BPUse = 2
	oldRecovery := s.bpNextRecovery
	if err := s.chargeMultiplayerStart(start, release.State{}, func(release.State) error { return errors.New("disk unavailable") }); err == nil || s.bp != 5 || len(s.teamBattleStartReceipts) != 1 || !s.bpNextRecovery.Equal(oldRecovery) {
		t.Fatal("failed persistence did not roll back debit")
	}
}

func TestMultiplayerContinueDebitIsDurableAndAtomic(t *testing.T) {
	s := &store{coin: 102, coinFree: 20}
	request := multiplayer.BattleContinue{RoomID: 6020000001, BossID: 30010102, UserID: 1000001, Sequence: 1}
	var saved release.State
	writes := 0
	persist := func(state release.State) error { saved = state; writes++; return nil }
	if _, err := s.chargeMultiplayerContinue(request, release.State{}, persist); err != nil {
		t.Fatal(err)
	}
	...
	restored := &store{coin: saved.User.Coin, coinFree: saved.User.CoinFree, teamBattleContinueReceipts: saved.TeamBattleContinueReceipts}
	if _, err := restored.chargeMultiplayerContinue(request, release.State{}, persist); err != nil || writes != 1 {
		t.Fatal("durable retry charged twice")
	}
	request.Sequence++
	if _, err := s.chargeMultiplayerContinue(request, release.State{}, func(release.State) error { return errors.New("disk unavailable") }); err == nil || s.coin != 72 || s.coinFree != 0 || len(s.teamBattleContinueReceipts) != 1 {
		t.Fatal("failed debit did not roll back")
	}
	if _, err := s.chargeMultiplayerContinue(request, release.State{}, persist); err != nil || s.coin != 22 || writes != 2 {
...
func TestPrepaidHostAndFreeGuestCanSettleAtZeroBP(t *testing.T) {
	...
	for _, host := range []bool{true, false} {
		...
		if host {
			roomID = 602000001
			hostType = 1
			s.teamBattleStartReceipts = []release.TeamBattleStartReceipt{{RoomID: roomID, BossID: boss, BPUse: 8}}
		}
