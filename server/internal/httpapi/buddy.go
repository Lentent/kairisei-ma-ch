...
type buddyFusionResult struct {
	...
	Decks          []deckInfo
	BackUniqueIDs  []int64
	BackStackCards []release.CardStack
}
...
func (s *store) buddyState() []release.Buddy {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := append([]release.Buddy(nil), s.buddies...)
...
func (s *store) buddyIndexByUniqueIDLocked(uniqueID int64) int {
...
func (s *store) clearBuddyIDsFromDecksLocked(remove map[int64]struct{}) {
	if len(remove) == 0 {
		return
	}
...
func (s *store) normalizeBuddyLocked(buddy release.Buddy, definition release.BuddyDefinition) (release.Buddy, error) {
	if buddy.Level < 1 || buddy.Level > definition.MaxLevel || buddy.Experience < 0 {
		return release.Buddy{}, errors.New("buddy level or experience is invalid")
	}
	values, exists := s.buddyExperience[definition.ExperienceTableID]
	if !exists || len(values) < definition.MaxLevel-1 {
		return release.Buddy{}, errors.New("buddy experience table is unavailable")
	}
	spent := 0
	for level := 1; level < buddy.Level; level++ {
		if spent > math.MaxInt-values[level-1] {
			return release.Buddy{}, errors.New("buddy experience table overflows")
		}
	...
	if buddy.Level < definition.MaxLevel {
		...
		if nowExperience < 0 || nowExperience >= required {
			return release.Buddy{}, errors.New("buddy experience does not match its level")
		}
		nextExperience = required - nowExperience
	} else if buddy.Experience != spent {
		return release.Buddy{}, errors.New("maximum-level buddy experience is invalid")
	}
...
func (s *store) buddyMaximumExperienceLocked(definition release.BuddyDefinition) (int, error) {
	values, exists := s.buddyExperience[definition.ExperienceTableID]
	if !exists || len(values) < definition.MaxLevel-1 {
...
func (s *store) addBuddyExperienceLocked(buddy release.Buddy, definition release.BuddyDefinition, addition int) (release.Buddy, error) {
	if addition <= 0 || buddy.Level >= definition.MaxLevel || buddy.Experience > math.MaxInt-addition {
		return release.Buddy{}, errors.New("buddy cannot gain experience")
	}
	maximum, err := s.buddyMaximumExperienceLocked(definition)
	if err != nil {
		return release.Buddy{}, err
	}
...
func (s *store) fuseBuddy(baseUniqueID int64, inputs []buddyFusionInput) (buddyFusionResult, error) {
	...
	stackUses := make([]release.CardStackUse, 0, len(seenStacks))
	...
	for _, value := range prepared {
		...
		if consume > 0 {
			stackUses = append(stackUses, release.CardStackUse{CardID: int(value.input.ID), Num: consume})
	...
	kept := make([]release.Buddy, 0, len(s.buddies)-len(consumedBuddies))
	...
	s.gold -= int(cost)
	backStackCards := make([]release.CardStack, 0)
	for cardID, count := range backStackCounts {
		if count == 0 {
			continue
		}
		var current release.CardStack
...
func (s *store) prepareBuddyEvolutionMaterialLocked(base release.Buddy, materialUniqueID int64, materialCardID int) (buddyEvolutionMaterial, error) {
	...
	if materialUniqueID != 0 {
		...
		materialDefinition := release.BuddyDefinition{}
		if index >= 0 {
...
func (s *store) consumePreparedBuddyEvolutionMaterialLocked(material buddyEvolutionMaterial) {
...
func (s *store) evolveBuddy(baseUniqueID int64, materialUniqueID int64, materialCardID int) (buddyEvolutionResult, error) {
	if baseUniqueID <= 0 {
	...
	if definition.EvolutionID == 0 {
		...
		if !exists {
			item = release.Item{ItemID: definition.OverlimitItemID}
		}
...
func (s *store) removeBuddiesLocked(remove map[int64]struct{}) {
	if len(remove) == 0 {
		return
	}
	kept := make([]release.Buddy, 0, len(s.buddies)-len(remove))
	for _, buddy := range s.buddies {
...
func (s *store) sellBuddies(uniqueIDs []int64) (int, int, int, []deckInfo, error) {
	if len(uniqueIDs) == 0 {
...
func (s *store) setBuddyLock(uniqueID int64, locked bool) error {
	if uniqueID <= 0 {
...
func (s *store) validateDeckBuddies(deck deckInfo) error {
...
func (a *API) buddyFusion(writer http.ResponseWriter, request *http.Request) {
	var payload struct {
		...
		Inputs       []buddyFusionInput `json:"add_inputs"`
	}
	if err := decodeExact(request, []string{"base_uniqid", "add_inputs"}, &payload); err != nil {
		a.writeStoreError(writer, err)
		return
	}
	result, err := a.store.fuseBuddy(payload.BaseUniqueID, payload.Inputs)
	if err != nil {
		a.writeStoreError(writer, err)
		return
	}
...
func (a *API) buddyEvolution(writer http.ResponseWriter, request *http.Request) {
	...
	result, err := a.store.evolveBuddy(payload.BaseUniqueID, payload.MaterialUniqueID, payload.MaterialCardID)
	if err != nil {
		a.writeStoreError(writer, err)
		return
	}
...
func (a *API) buddySell(writer http.ResponseWriter, request *http.Request) {
	...
	getGold, gold, count, decks, err := a.store.sellBuddies(payload.UniqueIDs)
	if err != nil {
		a.writeStoreError(writer, err)
		return
	}
...
func (a *API) setBuddyLock(writer http.ResponseWriter, request *http.Request, locked bool) {
	...
	if err := a.store.setBuddyLock(payload.UniqueID, locked); err != nil {
		a.writeStoreError(writer, err)
		return
	}
	if !a.persistOrError(writer) {
