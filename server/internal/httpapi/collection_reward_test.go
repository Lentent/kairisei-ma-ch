...
func TestCollectionRewardExchangeAndDuplicateGift(t *testing.T) {
	for _, kind := range []int{14, 16, 18} {
		s := &store{costumeIDs: map[int]struct{}{}, stampIDs: map[int]struct{}{}, honorIDs: map[int]struct{}{},
			collectionRewardIDs: map[[2]int]struct{}{{kind, 2}: {}}, items: map[int]release.Item{4000: {ItemID: 4000, Num: 100}},
			tradeShopPurchases: map[int]int{}, tradeShopProfiles: map[int]release.TradeShopProfile{1: {TradeShopID: 1, Lineups: []release.TradeShopLineupProfile{{LineupID: 2, LineupName: "解锁", StockNum: 1, Prices: []release.TradeShopPointProfile{{Type: 4, ID: 4000, Num: 25}}, Rewards: []release.Reward{{Type: kind, RewardTypeID: 2, Num: 1}}}}}}}
		if _, err := s.buyTradeShop(2, 1, nil); err != nil {
			t.Fatal(err)
		}
		reward := release.Reward{Type: kind, RewardTypeID: 2, Num: 1}
...
func TestConfiguredFamePoolKeepsTriggerAndFrozenRewards(t *testing.T) {
	policy := release.TeamBattleFameBonusPolicy{ConfigVersion: 1, ChanceMaximum: 100, FullFameThreshold: 100, FullFameRewardCount: 2, RewardSource: "first_inventory_result_reward", RollPolicy: "sha256_seed_modulo_chance_maximum_plus_one", EligibleRewardTypes: []int{6, 13, 8}}
	s := &store{teamBattleFameBonus: policy}
	profile := release.TeamBattleRewardProfile{ResultRewards: []release.Reward{{Type: 13, RewardTypeID: 1, Num: 1}}, FameRewards: []release.Reward{{Type: 8, RewardTypeID: 4000, Num: 600}}}
	...
	profile.FameRewards = []release.Reward{}
