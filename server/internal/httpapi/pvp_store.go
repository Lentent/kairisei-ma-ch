...
func (s *store) refreshPVPChallenges(config PVPConfig, now time.Time, base release.State, persist StatePersister) error {
...
func (s *store) pvpStatus() (int, int) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.pvpPoint, s.pvp.Challenge
}

func (s *store) pvpStatusWithBattleID() (int, int, int) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *store) validatePVPDefenseLocked(decks []release.PVPDeckSelection) ([]release.PVPDeckSelection, error) {
	if len(decks) != 4 {
...
func (s *store) beginPVP(battleType int, opponentUserID int, decks []release.PVPDeckSelection, config PVPConfig, now time.Time) (release.PVPMatch, int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	canonical, err := s.validatePVPDefenseLocked(decks)
	if err != nil {
		return release.PVPMatch{}, s.pvp.Challenge, fmt.Errorf("%w: %v", errInvalidPVPDeck, err)
	}
	if s.pvp.ActiveMatch != nil {
		return release.PVPMatch{}, s.pvp.Challenge, errors.New("当前竞技场对战尚未结束。")
	}
	if opponentUserID <= 0 {
		return release.PVPMatch{}, s.pvp.Challenge, errors.New("PVP opponent is invalid")
	}
	if battleType == 1 {
		if s.pvp.Challenge <= 0 {
			return release.PVPMatch{}, 0, errors.New("排位战挑战次数已用完，可选择自由战。")
		}
		s.pvp.Challenge--
	}
	...
	if len(s.pvp.History) > pvpHistoryLimit {
		s.pvp.History = append([]release.PVPMatch(nil), s.pvp.History[len(s.pvp.History)-pvpHistoryLimit:]...)
	}
	s.pvp.ActiveMatch = nil
	return match, s.pvp.Challenge, nil
}

func (s *store) finishPVP(battleID int, isWin bool, isRetire bool, config PVPConfig, now time.Time) (release.PVPMatch, int, int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.pvp.ActiveMatch == nil || s.pvp.ActiveMatch.BattleID != battleID {
		...
		return release.PVPMatch{}, s.pvp.Challenge, 0, errors.New("PVP settlement does not match the active battle")
	}
	match := *s.pvp.ActiveMatch
	if config.EngineMode == pvpEngineServerReplay && !isRetire && isWin != match.ExpectedWin {
		return release.PVPMatch{}, s.pvp.Challenge, 0, errors.New("PVP client result differs from the authoritative replay")
	}
...
func settleNativePVPMatch(match release.PVPMatch, isWin bool, isRetire bool, config PVPConfig, now time.Time) release.PVPMatch {
