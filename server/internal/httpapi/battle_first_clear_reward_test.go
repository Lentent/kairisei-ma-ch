...
func TestQuestFirstClearCrystalsAreGrantedOncePerDifficulty(t *testing.T) {
	for _, stageQuest := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			s := &store{coinFree: 7, teamBattleSolo: configuration}
			if stageQuest {
			...
			for _, step := range []struct {
				...
				profile := release.TeamBattleRewardProfile{BossID: step.boss, FirstClearRewards: []release.Reward{{Type: 10, Num: 50, CardSkillLevels: []int16{}}}}
				if stageQuest {
				...
				result, err := s.completeTeamBattle(step.boss, step.win, []release.TeamBattleRewardProfile{profile})
				if err != nil {
					t.Fatal(err)
				}
